...
# Deep Learning Modules for Enhanced Analysis
try:
    from dnn_modules import BERTEntityExtractor, SkillExtractor, ResumeClassifier, NeuralRecommender
    DNN_ENABLED = True
    print("[SUCCESS] DNN modules loaded successfully!")
except Exception as e:
    print(f"[WARNING] DNN modules not available: {e}")
    DNN_ENABLED = False
...
# Initialize DNN models (cached for performance)
@st.cache_resource(show_spinner="Loading Deep Learning models... (First time: ~30 seconds)")
def initialize_dnn_models():
    """Initialize all DNN models once and cache them"""
    if not DNN_ENABLED:
        return None
    ...
    try:
        ...
        models['bert_extractor'] = BERTEntityExtractor()
        
        # Skill Extractor (uses BERT)
        models['skill_extractor'] = SkillExtractor()
        
        # Resume Classifier (uses BERT)
        models['resume_classifier'] = ResumeClassifier()
        
        # Neural Recommender (uses Sentence Transformers)
        models['neural_recommender'] = NeuralRecommender()
        
        load_time = time.time() - start_time
        print(f"[INFO] DNN models loaded in {load_time:.2f} seconds")
        
        return models
    except Exception as e:
        st.warning(f"Could not initialize DNN models: {e}")
        print(f"[ERROR] DNN initialization failed: {e}")
        return None
...
# sql connector
connection = pymysql.connect(host='localhost',user='root',password='Keepcalm@2005',db='cv')
...
def insert_data(sec_token,ip_add,host_name,dev_user,os_name_ver,latlong,city,state,country,act_name,act_mail,act_mob,name,email,res_score,timestamp,no_of_pages,reco_field,cand_level,skills,recommended_skills,courses,pdf_name):
    ...
    (sec_token, ip_add, host_name, dev_user, os_name_ver, latlong, city, state, country, act_name, act_mail, act_mob, name, email, res_score, timestamp, no_of_pages, reco_field, cand_level, skills, recommended_skills, courses, pdf_name)
    ...
    try:
        ...
        st.warning(f"Note: Could not save data to database. Error: {str(e)}")
        connection.rollback()
...
def insertf_data(feed_name,feed_email,feed_score,comments,Timestamp):
    ...
    try:
        ...
        st.warning(f"Note: Could not save feedback to database. Error: {str(e)}")
        connection.rollback()
...
def run():
    ...
    if choice == 'User':
        ...
        # DNN Toggle
        if DNN_ENABLED:
            use_dnn = st.checkbox("🧠 Enable Deep Learning Analysis (BERT, CNN, BiLSTM)", value=True, 
                                  help="First time loading takes ~30 seconds to download models. Subsequent loads are instant.")
            if use_dnn:
                st.info("ℹ️ **First-time users**: DNN models will download (~500MB) and load (~30 sec). After that, it's instant!")
        else:
            use_dnn = False
        ...
        if pdf_file is not None:
            ...
            if resume_data:
                ...
                ## Initialize DNN models (only if enabled)
                dnn_models = None
                if use_dnn and DNN_ENABLED:
                    dnn_models = initialize_dnn_models()
                ...
                if use_dnn and dnn_models:
                    ...
                    with st.spinner('Running BERT entity extraction...'):
                        try:
                            bert_entities = dnn_models['bert_extractor'].extract_all_entities(resume_text)
                            
                            col1, col2 = st.columns(2)
                            with col1:
                                st.subheader("**🎯 BERT Extracted Entities**")
                                if bert_entities.get('organizations'):
                                    st.write("**Organizations:**", ', '.join(bert_entities['organizations'][:5]))
                                if bert_entities.get('locations'):
                                    st.write("**Locations:**", ', '.join(bert_entities['locations'][:3]))
                        except Exception as e:
                            st.info(f"BERT extraction: {e}")
                    
                    # CNN Resume Classification
                    with st.spinner('Classifying resume with CNN...'):
                        try:
                            classification = dnn_models['resume_classifier'].classify_resume(resume_text)
                            
                            with col2:
                                st.subheader("**🎓 Job Category Prediction**")
                                st.success(f"**Predicted Role:** {classification['predicted_category']}")
                                st.metric("Confidence", f"{classification['confidence']*100:.1f}%")
                                
                                with st.expander("View Top 3 Predictions"):
                                    for pred in classification['top_3_predictions']:
                                        st.write(f"• {pred['category']}: {pred['confidence']*100:.1f}%")
                        except Exception as e:
                            st.info(f"Classification: {e}")
                    
                    # BiLSTM Skill Extraction
                    with st.spinner('Extracting skills with BiLSTM...'):
                        try:
                            dnn_skills = dnn_models['skill_extractor'].extract_skills_hybrid(resume_text)
                            
                            st.subheader("**💼 Enhanced Skill Analysis**")
                            if dnn_skills:
                                skill_categories = dnn_models['skill_extractor'].get_skills_by_category(dnn_skills)
                                
                                tabs = st.tabs(list(skill_categories.keys())[:5])
                                for i, (category, skills) in enumerate(list(skill_categories.items())[:5]):
                                    with tabs[i]:
                                        st.write(", ".join(skills))
                                
                                st.info(f"Total skills detected: {len(dnn_skills)}")
                        except Exception as e:
                            st.info(f"Skill extraction: {e}")
                ...
                if use_dnn and dnn_models:
                    st.markdown("---")
                    st.header("**🎯 AI-Powered Job & Course Recommendations**")
                    
                    # Job Recommendations
                    with st.spinner('Finding best job matches using neural embeddings...'):
                        try:
                            job_recommendations = dnn_models['neural_recommender'].recommend_jobs(resume_text, top_k=5)
                            
                            if job_recommendations:
                                st.subheader("**💼 Top Job Matches for You**")
                                for i, job in enumerate(job_recommendations, 1):
                                    with st.expander(f"#{i} {job['job_title']} - {job['match_percentage']}% Match"):
                                        st.write(f"**Description:** {job['description']}")
                                        st.write(f"**Required Skills:** {', '.join(job['required_skills'])}")
                                        st.write(f"**Experience Level:** {job['experience_level']}")
                                        st.progress(job['similarity_score'])
                        except Exception as e:
                            st.info(f"Job recommendations: {e}")
                    
                    # Skill Gap Analysis
                    with st.spinner('Analyzing skill gaps...'):
                        try:
                            if job_recommendations and len(job_recommendations) > 0:
                                target_job = job_recommendations[0]['job_title']
                                current_skills = resume_data.get('skills', [])
                                
                                skill_gaps = dnn_models['neural_recommender'].find_skill_gaps(current_skills, target_job)
                                
                                if skill_gaps:
                                    st.subheader("**📊 Skill Gap Analysis**")
                                    st.warning(f"Skills to develop for {target_job}:")
                                    cols = st.columns(3)
                                    for idx, skill in enumerate(skill_gaps[:9]):
                                        with cols[idx % 3]:
                                            st.info(f"🎯 {skill}")
                        except Exception as e:
                            st.info(f"Skill gap analysis: {e}")
                    
                    # Neural Course Recommendations
                    with st.spinner('Recommending courses using semantic similarity...'):
                        try:
                            course_recommendations = dnn_models['neural_recommender'].recommend_courses(
                                resume_text, 
                                skills_gap=skill_gaps if 'skill_gaps' in locals() else None,
                                top_k=5
                            )
                            
                            if course_recommendations:
                                st.subheader("**📚 Personalized Course Recommendations**")
                                for i, course in enumerate(course_recommendations, 1):
                                    with st.expander(f"#{i} {course['course_name']} - {course['match_percentage']}% Relevant"):
                                        st.write(f"**Description:** {course['description']}")
                                        st.write(f"**Platform:** {course['platform']}")
                                        st.write(f"**Duration:** {course['duration']}")
                                        st.write(f"**Level:** {course['level']}")
                                        st.progress(course['relevance_score'])
                        except Exception as e:
                            st.info(f"Course recommendations: {e}")
